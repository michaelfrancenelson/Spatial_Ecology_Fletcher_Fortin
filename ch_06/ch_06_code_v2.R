...
#  ---- varied_thrush_data ----
{
  ...
  thrush_dt = cbind(thrush_dt, thrush_dt[, extract(dat_brick, thrush_dt[, .(x = EASTING, y = NORTHING)])])
  # 
  # thrush_sf = st_as_sf(
  #   thrush_dt,
...
## ---- center_data ----
{
  thrush_dt$elevation_sc = scale(thrush_dt$elevation, center = T, scale = T)
  thrush_dt$slope_sc = scale(thrush_dt$slope, center = T, scale = T)
  thrush_dt$aspect_sc = scale(thrush_dt$aspect, center = T, scale = T)
}
...
# ---- spaced_samples_model ----
{
  ...
  dat_2 = thrush_dt[dat[, SURVEYID[sample(length(TRANSECT), 1)], by = TRANSECT]$V1, ]
...
## ---- model_1_diagnostics ----
{
  dat_fit_1 = data.frame(thrush_dt)
...
## ---- model_3_diagnostics ----
{
  dat_fit_3 = data.frame(thrush_dt)
...
# ---- model_plots ----
{
  newdata = data.frame(
    elevation_sc =   
      seq(
        min(c(data.frame(thrush_dt)$elevation_sc)), 
        max(c(data.frame(thrush_dt)$elevation_sc)), 
        length = 25))
  ...
  pred_elev_poly_sc = predict(
    fit_elev_poly_sc, 
    newdata = newdata, 
    type =  "link", 
    se = T) 
  
  pred_elev_sc = predict(
    fit_elev_sc, 
    newdata = newdata, 
    type =  "link", 
    se = T) 
  ...
  ggplot(data.frame(pred_elev_poly_sc), 
...
## ---- census_histograms ----
{
  ...
  pplot = as.ggplot(
    ~pairs.panels(
      data.frame(thrush_dt)[, c("elevation", "slope", "aspect")]))
...
## ---- terrain_histograms ----
{
  gm_hist_thrush = ggplot(thrush_dt) + gm_thrush_hist_2
  ...
  gm_hist_thrush_present = 
    ggplot(subset(thrush_dt, VATH == 1)) +
    gm_thrush_hist_3
  gm_hist_thrush_absent = 
    ggplot(subset(thrush_dt, VATH == 0)) +
    gm_thrush_hist_3
