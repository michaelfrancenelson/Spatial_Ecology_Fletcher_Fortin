...
if (execute)
{
  ...
  # thrush_inset_map = gg_map + geom_stars(data = elev_inset, show.legend = TRUE) +
  # thrush_inset_map = gg_map + geom_stars(data = elev_inset) +
  thrush_inset_map = gg_map + geom_stars(data = elev_inset, show.legend = FALSE) +
    ...
    # geom_sf(aes(colour = present), data = thrush_inset, size = 2, show.legend = FALSE) +
    labs(fill = "Elevation") + cols + sc_census + theme(legend.title = element_blank())
  # labs(fill = "Elevation") + cols + sc_census + theme(legend.title = element_blank(), legend.background = element_blank())
  
  # thrush_inset_map
  # thrush_map
  
  # print(plot_grid(thrush_map + inset, thrush_inset_map, nrow = 1))
...
if (execute)
{
  if (save_images)
  {
    ...
    #     guide = map_cb)
        ...
        # dat_br[, , , 1]
        # 
        # range(dat_br$elevation, na.rm = TRUE)
        # 
        # hist(dat_br$elevation)
        # hist(terrain_brick[, , , 1]$elevation)
        # 
        # 
        # 
        # 
        ...
        # cols = gg_map_thrush
        # cols = gg_map_grayscale
        # cols = gg_map_elevation
        # 
        # 
        # 
        # 
        # 
        # thrush_inset_map = gg_map + geom_stars(data = elev_inset, show.legend = FALSE) + labs(fill = "Elevation") + cols +
        #   geom_sf(aes(colour = present), data = thrush_inset, size = 2, pch = 16, show.legend = "point") +
        #   geom_sf(aes(colour = present), data = thrush_inset, size = 2, pch = 16, show.legend = FALSE) +
        #   theme(legend.title = element_blank(), legend.background = element_blank()) + sc_census
        # 
        # thrush_map = gg_map + geom_stars(data = dat_br[, , , 1]) + labs(fill = "Elevation") + cols +
        #   geom_sf(aes(colour = present), data = thrush_sf, size = 3, pch = 16, show.legend = FALSE) +
        #   geom_sf(aes(colour = present), data = thrush_sf, size = 3, pch = 16, show.legend = FALSE) + sc_census
        # 
        # 
        # 
        # 
        ...
        # # scale_color_manual(values = c(2, 3), guide = guides(colour = guide_legend(override.aes = list(shape = 22))) )
        # thrush_inset_map
        #
        ...
        png(file.path(fig_dir, "varied_thrush_inset.png"), units = "in", height = map_panel_height, width = map_panel_width, res =  map_res)
        print(thrush_inset_map)
        dev.off()

        png(file.path(fig_dir, "varied_thrush_map.png"), units = "in", height = map_panel_height, width = map_panel_width, res =  map_res)
        print(thrush_map)
        dev.off()
        
        
        png(file.path(fig_dir, "varied_thrush_map_and_inset.2.png"), units = "in", height = map_panel_height, width = 2 * map_panel_width, res =  map_res)
        print(plot_grid(thrush_map + inset, thrush_inset_map, nrow = 1))
        dev.off()
